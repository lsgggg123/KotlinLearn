...
/*
    带接受者的函数字面值
    Kotlin 提供了这样一种功能，可以通过指定的接受者对象来调用一个函数字面值
...
fun main() {
    // 这里的 Int. 代表 Int 类型可以调这个函数
    //            Int.
    val subtract: Int.(other: Int) -> Int = { other -> this - other }
    println(3.subtract(1))
    println("--------------------")

    // 匿名函数语法可以让我们指定函数字面值的接受者类型，这样我们就可以先去声明一个带有接受者的函数类型变量，然后再去使用它
    val sum = fun Int.(other: Int): Int = this + other
    println(1.sum(2))
    println("-----------------")

    // 带有接受者类型的函数的非字面值可以作为参数进行传递，前提是所需要接受函数的地方应该有一个接受者类型的参数，反之亦然。
